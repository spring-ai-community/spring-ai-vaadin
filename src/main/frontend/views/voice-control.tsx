...
export default function VoiceControlView() {
  ...
  const { field, model, submit, read, clear } = useForm(IssueModel, {
    onSubmit: async (issue: Issue) => {
      ...
      issues.value = issues.value.map((i) => (i.id === updatedIssue.id ? updatedIssue : i));
    },
  });
      ...
      {selectedIssue.value && (
        <FormLayout>
          ...
          <Select
            ...
            {...field(model.status)}
          />
          <TextField label="Assignee" {...field(model.assignee)} />
          <div className="flex gap-m py-m">
            ...
            {selectedIssue.value.id !== 0 && (
              <Button theme="error" onClick={() => functions.find((f) => f.name === 'deleteCurrentIssue')?.execute({})}>
                Delete
              </Button>
            )}
            <Button
