...
public class Chat extends VerticalLayout {
    ...
    public Chat() {
        ...
        upload = new Upload(UploadHandler.inMemory((meta, data) -> {
            ...
            if (isImage) {
                ...
                pendingAttachments.add(new ChatAttachment(meta.contentType(), meta.fileName(), data, dataUrl));
            } else {
                pendingAttachments.add(new ChatAttachment(meta.contentType(), meta.fileName(), data, ""));
            }
        }));
        ...
        public ChatMessage(String role, String content, List<ChatAttachment> attachments) {
            ...
            if (attachments != null) {
                ...
                for (var attachment : attachments) {
                    ...
                    if (isImage) {
                        attachmentsBuilder.append("<img src='" + attachment.url + "' alt='" + attachment.fileName + "' class='attachment-image' />");
                    } else {
                        attachmentsBuilder.append("<div class='attachment-icon'>📄</div>");
                    };
                    attachmentsBuilder.append("<span class='attachment-name'>" + attachment.fileName + "</span>");
