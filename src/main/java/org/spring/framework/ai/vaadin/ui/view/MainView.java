...
public class MainView extends MasterDetailLayout {
    ...
    public MainView(@Autowired Assistant assistant, @Autowired RagContextService ragContextService) {
        ...
        chat.setSizeFull();
        chat.setSubmitListener((ChatMessage userMessage, ChatMessage assistantMessage) -> {
            chat.setEnabled(false);
            options = new ChatOptions(settingsPanel.getSystemMessage(), settingsPanel.isUseMcp());
            var attachmentFiles = userMessage.getAttachments().stream().map(this::chatAttahcmentToAttachmentFile).toList();
            assistant.stream(chatId, userMessage.getContent(), attachmentFiles, options).subscribe((String token) -> {
    ...
    private ChatMessage messageToChatMessage(Message message) {
        return new ChatMessage(message.role(), message.content(), message.attachments().stream().map(
                a -> new ChatAttachment(a.type(), a.fileName(), null, a.url())
        ).toList());
    }
    
    private AttachmentFile chatAttahcmentToAttachmentFile(ChatAttachment chatAttachment) {
        return new AttachmentFile(chatAttachment.fileName(), chatAttachment.type(), chatAttachment.data());
    }
